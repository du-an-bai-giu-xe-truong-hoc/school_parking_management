...
class XeRaPage(ctk.CTkFrame):
    ...
    def _build_right_panel(self):
        ...
        self.entry_image_label.pack(anchor="center", padx=10, pady=(0, 8))
        ...
        self.exit_image_label.pack(anchor="center", padx=10, pady=(0, 10))
    ...
    def _apply_exit_detection(self, payload: dict):
        ...
        if detection_type == "ir":
            self._clear_form()
            self.capture_status.configure(text="IR sensor kich hoat: da reset giao dien xe ra.")
            return
        ...
        if detection_type == "text" and confidence >= 70.0:
            if self._looks_like_plate_text(value):
                ...
                if value == last_barcode and (current_time - last_time) < 30:
                    self.capture_status.configure(text=f"Bo qua OCR text {value} (Debounce 30s)")
                    return
        ...
        if detection_type in ("refc", "qr") and confidence >= 70.0:
            ...
            last_barcode = getattr(self, "last_scanned_barcode", "")
            if value == last_barcode and (current_time - last_time) < 30:
                self.capture_status.configure(text=f"Bo qua {detection_type.upper()} {value} (Debounce 30s)")
                return
            self.last_scanned_barcode = value
            self.last_scanned_time = current_time
    ...
    def _set_exit_barcode(self, value: str):
        ...
        last_barcode = getattr(self, "last_scanned_barcode", "")
        if value == last_barcode and (current_time - last_time) < 30:
            self.capture_status.configure(text=f"Bo qua barcode {value} (Debounce 30s)")
            return
        self.last_scanned_barcode = value
        self.last_scanned_time = current_time
    ...
    def _preview_vehicle(self):
        ...
        preview = ApiClient.preview_vehicle(qr_code=qr_code, bien_so=self.exit_plate.get().strip())
        if preview is None:
            return
    ...
    def _emergency_open(self):
        if messagebox.askyesno("Xác nhận", "Bạn có chắc chắn muốn mở cổng khẩn cấp không? Hành động này sẽ được ghi log."):
            try:
                lane = self.exit_lane.get()
                ApiClient.emergency_open(lane=lane, gate_type="RA")
                self.hardware.open_exit_gate()
                self.capture_status.configure(text="ĐÃ MỞ CỔNG KHẨN CẤP!")
