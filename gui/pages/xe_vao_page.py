...
class XeVaoPage(ctk.CTkFrame):
    ...
    def _build_camera_panel(self):
        ...
        self.local_camera_label.pack(anchor="center", padx=14, pady=(0, 10))
        ...
        if use_backend_proxy:
            ...
            if not prefix.startswith("/"):
                prefix = f"/{prefix}"
            iot_capture_url = f"{backend}{prefix}/camera/esp32/frame"
        else:
            iot_capture_url = os.getenv("ESP32_CAPTURE_URL") or f"http://{os.getenv('ESP32_IP', '192.168.101.8')}/capture"
        ctk.CTkLabel(panel, text=f"IoT ESP32 camera ({iot_capture_url})", font=AppStyle.BODY_FONT).pack(
            anchor="w", padx=14, pady=(0, 4)
        )
        ...
        self.iot_camera_label.pack(anchor="center", padx=14, pady=(0, 10))
    ...
    def _build_form_panel(self):
        ...
        self.chk_manual_override = ctk.CTkCheckBox(
            panel, text="Nhập thủ công (Bỏ qua Camera OCR)", variable=self.manual_override_var
        )
        self.chk_manual_override.pack(anchor="w", padx=14, pady=(0, 6))
    ...
    def _apply_detection_payload(self, payload: dict):
        ...
        if detection_type == "ir":
            self._clear_form()
            self.camera_status_label.configure(text="IR sensor kich hoat: da reset giao dien xe vao.")
            return
        ...
        if detection_type == "plate" and confidence >= 45.0:
            if not getattr(self, "manual_override_var", None) or not self.manual_override_var.get():
        ...
        if detection_type == "text" and confidence >= 70.0:
            if self._looks_like_plate_text(value):
                if not getattr(self, "manual_override_var", None) or not self.manual_override_var.get():
                    current_plate = self.entry_plate.get().strip().upper()
                    if current_plate != value.upper():
                        self.entry_plate.delete(0, "end")
                ...
                last_time = getattr(self, "last_scanned_time", 0)
                last_barcode = getattr(self, "last_scanned_barcode", "")
                if value == last_barcode and (current_time - last_time) < 30:
                    self.camera_status_label.configure(text=f"Bo qua OCR text {value} (Debounce 30s)")
                    return
                self.last_scanned_barcode = value
        ...
        if detection_type in ("refc", "qr") and confidence >= 70.0:
            ...
            last_barcode = getattr(self, "last_scanned_barcode", "")
            if value == last_barcode and (current_time - last_time) < 30:
                self.camera_status_label.configure(text=f"Bo qua {detection_type.upper()} {value} (Debounce 30s)")
                return
            self.last_scanned_barcode = value
            self.last_scanned_time = current_time
    ...
    @staticmethod
    def _looks_like_plate_text(text: str) -> bool:
        cleaned = re.sub(r"[^A-Z0-9]", "", (text or "").upper())
        if len(cleaned) < 6 or len(cleaned) > 12:
            return False
        return bool(re.match(r"^\d{2}[A-Z]\d?\d{4,6}$", cleaned))
    ...
    def _set_barcode_from_camera(self, code: str):
        ...
        last_barcode = getattr(self, "last_scanned_barcode", "")
        if value == last_barcode and (current_time - last_time) < 30:
            self.camera_status_label.configure(text=f"Bo qua barcode {value} (Debounce 30s)")
            return
        self.last_scanned_barcode = value
        self.last_scanned_time = current_time
    ...
    def _emergency_open(self):
        if messagebox.askyesno("Xác nhận", "Bạn có chắc chắn muốn mở cổng khẩn cấp không? Hành động này sẽ được ghi log."):
            try:
                lane = self.lane_option.get()
                ApiClient.emergency_open(lane=lane, gate_type="VAO")
                self.hardware.open_entry_gate()
                self.camera_status_label.configure(text="ĐÃ MỞ CỔNG KHẨN CẤP!")
                messagebox.showinfo("Thành công", "Cổng đã mở.")
            except Exception as e:
                messagebox.showerror("Lỗi", f"Lỗi khi mở cổng khẩn cấp: {str(e)}")
